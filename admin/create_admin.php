<?php
 ...
 require_once("include/connection.php");
   ...
   if(isset($_POST['reg'])){
         ...
         $user_status = mysqli_real_escape_string($conn,$_POST['userStatus']);
	...
	$q_checkadmin = $conn->query("SELECT * FROM `admin_login` WHERE `admin_user` = '$user_email'") or die(mysqli_error());
		...
		if($v_checkadmin == 1){
			...
			$conn->query("INSERT INTO `admin_login` VALUES('', '$user_name', '$office', '$designation', '$user_email', '$user_password', '$user_status')") or die(mysqli_error());
			echo '
				<script type = "text/javascript">
