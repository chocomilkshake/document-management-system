...
if (isset($_POST['save'])) { // if save button on the form is clicked
  ...
  $user = $_POST['email'];
    ...
    if (!in_array($extension, ['pdf', 'doc', 'docx', 'txt', 'zip', 'rar'])) {
  ...
  $query=mysqli_query($conn,"SELECT * FROM `upload_files` WHERE `name` = '$filename'")or die(mysqli_error($conn));
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO upload_files (name, office, designation, size, download, timers, admin_status, email) VALUES ('$filename', '{$_SESSION["office"]}', '{$_SESSION["designation"]}', $size, 0, '$time', '{$_SESSION['user_status']}', '$user')";
